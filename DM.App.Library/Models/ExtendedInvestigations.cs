...
namespace DM.App.Library.Models
{
    public partial class Investigations
    {
        ...
        public IEnumerable<Models.Interfaces.ICategoryField> FieldsConfiguration
        {
            get
            {
                if (_fieldsConfiguration == null)
                {
                    ...
                    _fieldsConfiguration = ExternalDB.GetCategoriesFields(this.CategoryId, ExternalDB.EntityInvestigations);

                    if (_fieldsConfiguration.Count() > 0)
                    {
                        //
                        // this method is commented out by Andreas Kasapleris, 08.09.2016
                        //

                        // IEnumerable<CategoriesFieldsPerTaskType> categoriesFieldsPerTaskType = ExternalDB.GetCategoriesFieldsPerTaskType(_fieldsConfiguration.First().CategoryId, ExternalDB.EntityInvestigations, this.TaskTypeId);

                        //
                        // 08.09.2016, choosen method to do the job by Andreas Kasapleris (from ExternalDB.cs)
                        //
                        IEnumerable<CategoriesFields> categoriesFieldsInvestigations = ExternalDB.GetCategoriesFields(_fieldsConfiguration.First().CategoryId, ExternalDB.EntityInvestigations);


                        if (categoriesFieldsInvestigations.Count() > 0)
                        {
                            //
                            // 08.09.2016, Andreas Kasapleris, small modification in foreach
                            //
                            foreach (CategoriesFields item in categoriesFieldsInvestigations)
                            {
        ...
        {
            // 08.09.2016, commented out by Andreas Kasapleris
            // this.Task_Id = Guid.NewGuid();

            // 08.09.2016, Andreas Kasapleris
            this.InvestigationsId = Guid.NewGuid();
        ...
        public static void SaveFileAttachments(ExternalDBEntities db, Investigations entity, string[] fileNames, string userLoginName, Guid fieldId, string localFilesPath, List<string> deletedAttachments)
        {
